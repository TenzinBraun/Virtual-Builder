...
using Wilberforce.FinalVignette;
    ...
    public class FinalVignetteCommandBuffer_Delay : MonoBehaviour
    {
        ...
        public float VignetteInnerValueDistance = 1.0f;
        public float VignetteOuterValueDistance = 1.0f;
        ...
        public Material FinalVignetteMaterial;
        ...
        private void TrySetUniforms()
        {
			...
			//Debug.Log("Outer : " + VignetteOuterValueDistance);
			//Debug.Log("Inner : " + VignetteInnerValueDistance);
			//Debug.Log("Type : " + VignetteOuterValueDistance.GetType());
			Vector3 velocity = (transform.position - oldPos) / Time.deltaTime;
			//Debug.Log("Velocity :" + velocity);
			//var temp = 1 - (velocity.magnitude / 150);
			//Debug.Log("Temp : " + temp);
			//Debug.Log("TypeTemp : " + temp.GetType());
            ...
            if (delayCounter >= 5)
			{
				...
				{
                    VignetteOuterValueDistance = 1- (velocity.magnitude / maxVelocity);
                    VignetteOuterValueDistance = 1- (velocity.magnitude / maxVelocity);
                    VignetteInnerValueDistance = 0.0f;
				...
				{
					FinalVignetteMaterial.SetFloat("vignetteMaxDistance", 1.0f);
					FinalVignetteMaterial.SetFloat("vignetteMinDistance", 1.0f);
				}
				delayCounter = 0;
			}
			else
        ...
        {
            if (ShouldUseCommandBuffer() && (this is FinalVignetteCommandBuffer_Delay) && !(this is FinalVignette)) return;
            ...
            for (int i = 0; i < allComponents.Length; i++)
            {
                if (ShouldUseCommandBuffer() && (allComponents[i] == this))
                {
                    ...
                    newComponent = oldGameObject.AddComponent<FinalVignetteCommandBuffer_Delay>();
                    (newComponent as FinalVignetteCommandBuffer_Delay).SetParameters(parameters);
                    oldComponentIndex = i;
                    break;
                }
    ...
    [CustomEditor(typeof(FinalVignetteCommandBuffer_Delay))]
    ...
    #region Unity Utilities
        ...
        void OnEnable()
        {
            ...
            FinalVignetteCommandBuffer_Delay effect = (target as FinalVignetteCommandBuffer_Delay);
        Debug.Log(effect.GetComponent<Camera>());
        ...
        {
            var script = target as FinalVignetteCommandBuffer_Delay;
            ...
            if (script.pipelineFoldout)
            {
                ...
                EditorGUI.BeginDisabledGroup(script.IntegrationType == FinalVignetteCommandBuffer_Delay.Integration.ImageEffect);
        ...
        private void VignetteGUI(FinalVignetteCommandBuffer_Delay script)
        {
